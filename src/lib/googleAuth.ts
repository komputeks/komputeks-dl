...
function buildGoogleUrl(appName: string): string | null {
  ...
  const redirectUri = process.env.NEXT_PUBLIC_GOOGLE_AUTH_PROXY || process.env.VITE_GOOGLE_AUTH_PROXY;
...
export function signInWithGoogle(appName = 'Komputeks Downloads'): void {
  const url = buildGoogleUrl(appName);
  if (!url) {
    console.warn('[google-auth] Missing Google OAuth env vars');
    return;
  }
